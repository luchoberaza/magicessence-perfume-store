...
export async function POST(request: NextRequest) {
  if (!(await isAuthenticated())) return NextResponse.json({ error: "No autorizado" }, { status: 401 })
  const { name, slug, brand, description, category_id, category_ids, color_hex, featured, sale_by_order, encargue_price_int, is_active } = await request.json()
  ...
  const encarguePrice = encargue_price_int != null ? parseInt(String(encargue_price_int)) : null
  const rows = await sql`
    INSERT INTO products (name, slug, brand, description, color_hex, category_id, featured, sale_by_order, encargue_price_int, is_active)
VALUES (${name}, ${slug}, ${brand || null}, ${description || null}, ${hex}, ${primaryCategoryId || null}, ${featured || false}, ${sale_by_order || false}, ${Number.isFinite(encarguePrice) ? encarguePrice : null}, ${is_active !== false})
    RETURNING *
  `
...
export async function PUT(request: NextRequest) {
  if (!(await isAuthenticated())) return NextResponse.json({ error: "No autorizado" }, { status: 401 })
  const { id, name, slug, brand, description, category_id, category_ids, color_hex, featured, sale_by_order, encargue_price_int, is_active } = await request.json()
  const hex = normalizeHex(color_hex)
  if (color_hex && !hex) {
    return NextResponse.json({ error: "color_hex inválido (usa formato #RRGGBB)" }, { status: 400 })
  }
  ...
  color_hex = ${hex}, category_id = ${primaryCategoryId || null}, featured = ${featured || false}, sale_by_order = ${sale_by_order || false}, encargue_price_int = ${Number.isFinite(encarguePrice) ? encarguePrice : null}, is_active = ${is_active !== false}
